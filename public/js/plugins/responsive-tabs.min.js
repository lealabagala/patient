...
!function (window, document, $) {
    "use strict";
    var pluginName = "responsiveHorizontalTabs", defaults = {
        ...
        var $nav = this.$nav = $tabs.find(this.options.navSelector), $dropdown = this.$dropdown = $nav.find(this.options.dropdownSelector), $items = this.$items = $nav.find(this.options.itemSelector).filter(function () {
    ...
    Plugin.prototype = {
        init: function () {
            if (0 !== this.$dropdown.length) {
                ...
                var length = this.length = this.$items.length, dropWidth = this.dropWidth = this.$dropdown.width(), total = 0;
            ...
            if (!(this.breakpoints.length <= 0)) {
                ...
                if (i !== this.flag) {
                    if (this.$items.removeClass(activeClassName), this.$dropdownItems.removeClass(activeClassName), this.$dropdown.removeClass(activeClassName), i === this.breakpoints.length)this.$dropdown.hide(), this.$items.show(), this.$items.eq(active).addClass(activeClassName); else {
                        this.$dropdown.show();
