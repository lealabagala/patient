...
$.components.register("ace", {
    mode: "init", defaults: {}, init: function (context) {
        "undefined" != typeof ace && (ace.config.loadModule("ace/ext/language_tools"), $('[data-plugin="ace"]', context).each(function () {
            var id = $(this).attr("id"), mode = $(this).data("mode"), theme = $(this).data("theme"), editor = ace.edit(id);
            editor.container.style.opacity = "", mode && editor.session.setMode("ace/mode/" + mode), theme && editor.setTheme("ace/theme/" + theme), editor.setOption("maxLines", 40), editor.setAutoScrollEditorIntoView(!0), ace.config.loadModule("ace/ext/language_tools", function () {
                editor.setOptions({enableSnippets: !0, enableBasicAutocompletion: !0})
            })
        }))
    }
}), $.components.register("alertify", {
    mode: "api",
    defaults: {type: "alert", labels: {ok: "OK", cancel: "Cancel"}, delay: 5e3, buttonReverse: !1, buttonFocus: "ok"},
    api: function () {
        if ("undefined" != typeof alertify) {
            ...
            $(document).on("click.site.alertify", '[data-plugin="alertify"]', function () {
                ...
                switch (options.labelOk && (options.labels.ok = options.labelOk), options.labelCancel && (options.labels.cancel = options.labelCancel), alertify.set(options), options.type) {
                    case"alert":
                        alertify.alert(options.alertMessage);
                        break;
                    case"confirm":
                        alertify.confirm(options.confirmTitle, function (e) {
                            e ? alertify.success(options.successMessage) : alertify.error(options.errorMessage)
                        });
                        break;
                    case"prompt":
                        alertify.prompt(options.promptTitle, function (e, str) {
                            options.successMessage.replace("%s", str), options.errorMessage.replace("%s", str), e ? alertify.success(options.successMessage) : alertify.error(options.errorMessage)
    ...
    mode: "init", defaults: {namespace: "breadcrumb"}, init: function (context) {
        if ($.fn.asBreadcrumbs) {
    ...
    defaults: {namespace: "colorInputUi"},
    mode: "default"
    ...
    defaults: {
        ...
        components: {
            prev: {
                ...
                tpl: function () {
                    var lists = "", remainder = this.currentPage >= this.visible ? this.currentPage % this.visible : this.currentPage;
                    remainder = 0 === remainder ? this.visible : remainder;
                    for (var k = 1; remainder > k; k++)lists += '<li class="' + this.namespace + '-items" data-value="' + (this.currentPage - remainder + k) + '"><a href="javascript:void(0)">' + (this.currentPage - remainder + k) + "</a></li>";
                    ...
                    for (var i = this.currentPage + 1, limit = i + this.visible - remainder - 1 > this.totalPages ? this.totalPages : i + this.visible - remainder - 1; limit >= i; i++)lists += '<li class="' + this.namespace + '-items" data-value="' + i + '"><a href="javascript:void(0)">' + i + "</a></li>";
                    return lists
                }
            }
        }
    ...
    init: function (context) {
        if ($.fn.asPaginator) {
            ...
            $('[data-plugin="paginator"]', context).each(function () {
                var $this = $(this), options = $this.data(), total = $this.data("total");
                options = $.extend({}, defaults, options), $this.asPaginator(total, options)
            })
        }
    }
    ...
    init: function (context) {
        if ($.fn.asPieProgress) {
            ...
            $('[data-plugin="pieProgress"]', context).each(function () {
                ...
                options = $.extend(!0, {}, defaults, options), $this.asPieProgress(options)
            })
        }
    }
    ...
    mode: "default",
    defaults: {namespace: "spinnerUi", skin: null, min: "-10", max: 100, mousewheel: !0}
}), $.components.register("bootbox", {
    mode: "api", defaults: {message: ""}, api: function () {
        if ("undefined" != typeof bootbox) {
            ...
            $(document).on("click.site.bootbox", '[data-plugin="bootbox"]', function () {
                var $btn = $(this), options = $btn.data();
                if (options = $.extend(!0, {}, defaults, options), options.classname && (options.className = options.classname), "string" == typeof options.callback && $.isFunction(window[options.callback]) && (options.callback = window[options.callback]), options.type)switch (options.type) {
                    case"alert":
    ...
    mode: "default",
    defaults: {autoclose: !0}
}), $.components.register("maxlength", {
    mode: "default",
    defaults: {}
}), $.components.register("selectpicker", {
    mode: "default",
    defaults: {style: "btn-select", iconBase: "icon", tickIcon: "wb-check"}
    ...
    init: function (context) {
        if ($.fn.treeview) {
            ...
            $('[data-plugin="treeview"]', context).each(function () {
                var $this = $(this), options = $this.data();
                "string" == typeof options.source && $.isFunction(window[options.source]) ? (options.data = window[options.source](), delete options.source) : $.isFunction(options.souce) && (options.data = options.source(), delete options.source), options = $.extend(!0, {}, defaults, options), $this.treeview(options)
            })
        }
    }
}), $.components.register("buttons", {
    mode: "api", defaults: {}, api: function () {
        $(document).on("click.site.loading", "[data-loading-text]", function () {
            var $btn = $(this), text = $btn.text(), i = 20, loadingText = $btn.data("loadingText");
            $btn.text(loadingText + "(" + i + ")").css("opacity", ".6");
            var timeout = setInterval(function () {
                $btn.text(loadingText + "(" + --i + ")"), 0 === i && (clearInterval(timeout), $btn.text(text).css("opacity", "1"))
            }, 1e3)
    ...
    defaults: {
        ...
        $.fn.dataTable && $.fn.dataTable.TableTools && $.extend(!0, $.fn.dataTable.TableTools.classes, {
            ...
            buttons: {normal: "btn btn-outline btn-default", disabled: "disabled"},
    ...
    mode: "init", defaults: {persistent: !0}, init: function (context) {
        if ($.fn.formatter) {
            var ieOptions, defaults = $.components.getDefaults("formatter"), browserName = navigator.userAgent.toLowerCase();
            ieOptions = /msie/i.test(browserName) && !/opera/.test(browserName) ? {persistent: !1} : {}, $('[data-plugin="formatter"]', context).each(function () {
                var options = $.extend({}, defaults, ieOptions, $(this).data());
                options.pattern && (options.pattern = options.pattern.replace(/\[\[/g, "{{").replace(/\]\]/g, "}}")), $(this).formatter(options)
            })
        }
    }
    ...
    defaults: {
        ...
        angle: .12,
        lineWidth: .4,
        pointer: {length: .68, strokeWidth: .03, color: $.colors("blue-grey", 400)},
        limitMax: !0,
    ...
    init: function (context) {
        if (Gauge) {
            ...
            $('[data-plugin="gauge"]', context).each(function () {
                ...
                if (options = $.extend(!0, {}, defaults, options), 0 !== $canvas.length) {
    ...
    defaults: {
        ...
        angle: .3,
        lineWidth: .08,
        pointer: {length: .9, strokeWidth: .035, color: $.colors("blue-grey", 400)},
        limitMax: !1,
        colorStart: $.colors("blue-grey", 200),
        colorStop: $.colors("blue-grey", 200),
        strokeColor: $.colors("primary", 500),
    ...
    styles: [{
    ...
    }, {featureType: "poi", elementType: "all", stylers: [{color: "#ffffff"}]}, {
        featureType: "road",
        elementType: "labels.text.fill",
        stylers: [{color: $.colors("blue-grey", "700")}]
    }, {
        featureType: "administrative",
        elementType: "labels.text.fill",
        stylers: [{color: $.colors("blue-grey", "500")}]
    }, {
        featureType: "road.highway",
        elementType: "geometry.fill",
        stylers: [{color: $.colors("blue-grey", "300")}]
    }, {featureType: "road.arterial", elementType: "geometry.fill", stylers: [{color: "#e0e0e0"}]}, {
        featureType: "water",
        elementType: "geometry.fill",
        stylers: [{color: $.colors("blue-grey", "200")}]
        ...
        elementType: "all",
        stylers: [{color: $.colors("blue-grey", "200")}, {weight: .5}]
    }, {
    ...
    }, {featureType: "poi", elementType: "all", stylers: [{visibility: "off"}, {color: "#000000"}]}, {
        featureType: "poi",
        elementType: "labels.text",
        stylers: [{visibility: "on"}, {color: $.colors("blue-grey", "700")}]
    }, {
        featureType: "road.local",
        elementType: "labels.text",
        stylers: [{color: $.colors("blue-grey", "700")}]
        ...
        featureType: "transit",
        elementType: "all",
        stylers: [{color: $.colors("blue-grey", "100")}]
    }, {
        ...
        featureType: "road",
        elementType: "labels.text.fill",
        stylers: [{color: $.colors("blue-grey", "600")}]
    }, {
        ...
        stylers: [{visibility: "on"}, {hue: "#ffffff"}, {saturation: -100}, {lightness: 50}]
    }, {
        featureType: "water",
        elementType: "labels.text.stroke",
        ...
        elementType: "labels.text.stroke",
        stylers: [{color: "#ffffff"}]
    }, {featureType: "water", elementType: "labels.text.fill", stylers: [{color: $.colors("blue-grey", "600")}]}]
}), $.components.register("sortable", {
    defaults: {},
    mode: "default"
}), $.components.register("iCheck", {mode: "default", defaults: {}}), $.components.register("appear", {
    defaults: {},
    ...
    mode: "default",
    defaults: {
        step: ".steps .step, .pearls .pearl", templates: {
            buttons: function () {
                var options = this.options;
                return '<div class="wizard-buttons"><a class="btn btn-default btn-outline" href="#' + this.id + '" data-wizard="back" role="button">' + options.buttonLabels.back + '</a><a class="btn btn-primary btn-outline pull-right" href="#' + this.id + '" data-wizard="next" role="button">' + options.buttonLabels.next + '</a><a class="btn btn-success btn-outline pull-right" href="#' + this.id + '" data-wizard="finish" role="button">' + options.buttonLabels.finish + "</a></div>"
            }
        }
    }
    ...
    defaults: {
        ...
        zoomAnimate: !0,
        regionStyle: {
            initial: {fill: $.colors("primary", 600)},
            hover: {fill: $.colors("primary", 500)},
            selected: {fill: $.colors("primary", 800)},
            selectedHover: {fill: $.colors("primary", 500)}
        },
        markerStyle: {
            initial: {r: 8, fill: $.colors("red", 600), "stroke-width": 0},
            hover: {r: 12, stroke: $.colors("red", 600), "stroke-width": 0}
        }
    }
    ...
    mode: "default",
    defaults: {type: "image", closeOnContentClick: !0, image: {verticalFit: !0}}
    ...
    mode: "init", defaults: {}, init: function (context) {
        if ("undefined" != typeof $.fn.matchHeight) {
            ...
            $('[data-plugin="matchHeight"]', context).each(function () {
                var $this = $(this), options = $.extend(!0, {}, defaults, $this.data()), matchSelector = $this.data("matchSelector");
    ...
    mode: "default",
    defaults: {}
}), $.components.register("nestable", {mode: "default"}), $.components.register("owlCarousel", {
    mode: "default",
    defaults: {
        loop: !0,
        nav: !0,
    ...
    api: function () {
        ...
        $(".panel", context).each(function () {
            var $loading, $this = $(this), isFullscreen = !1, isClose = !1, isCollapse = !1, isLoading = !1, $fullscreen = $this.find('[data-toggle="panel-fullscreen"]'), $collapse = $this.find('[data-toggle="panel-collapse"]'), self = this, api = {
                load: function (callback) {
                    var type = $this.data("loader-type");
                    type || (type = "default"), $loading = $('<div class="panel-loading"><div class="loader loader-' + type + '"></div></div>'), $loading.appendTo($this), $this.addClass("is-loading"), $this.trigger("loading.uikit.panel"), isLoading = !0, $.isFunction(callback) && callback.call(self, this.done)
                }, done: function () {
                    ...
                    isCollapse !== !1 && ($this.removeClass("is-collapse"), $collapse.hasClass("wb-plus") && $collapse.removeClass("wb-plus").addClass("wb-minus"), $this.trigger("shown.uikit.panel"), isCollapse = !1)
                }, hideContent: function () {
                    isCollapse !== !0 && ($this.addClass("is-collapse"), $collapse.hasClass("wb-minus") && $collapse.removeClass("wb-minus").addClass("wb-plus"), $this.trigger("hidden.uikit.panel"), isCollapse = !0)
                }, toggleFullscreen: function () {
                    isFullscreen ? this.leaveFullscreen() : this.enterFullscreen()
                }, enterFullscreen: function () {
                    isFullscreen !== !0 && ($this.addClass("is-fullscreen"), $fullscreen.hasClass("wb-expand") && $fullscreen.removeClass("wb-expand").addClass("wb-contract"), $this.trigger("enter.fullscreen.uikit.panel"), isFullscreen = !0)
                }, leaveFullscreen: function () {
                    isFullscreen !== !1 && ($this.removeClass("is-fullscreen"), $fullscreen.hasClass("wb-contract") && $fullscreen.removeClass("wb-contract").addClass("wb-expand"), $this.trigger("leave.fullscreen.uikit.panel"), isFullscreen = !1)
                }, toggle: function () {
    ...
    mode: "init",
    defaults: {delimiter: ",", fill: [$.colors("primary", 400)], height: 22, max: null, min: 0, padding: .1, width: 44},
    init: function (context) {
        if ($.fn.peity) {
            var defaults = $.components.getDefaults("peityBar");
            $('[data-plugin="peityBar"]', context).each(function () {
                var $this = $(this), options = $this.data();
                if (options.skin && $.colors(options.skin)) {
    ...
    init: function (context) {
        if ($.fn.raty) {
            ...
            $('[data-plugin="rating"]', context).each(function () {
                var $this = $(this), options = $.extend(!0, {}, defaults, $this.data());
                options.hints && (options.hints = options.hints.split(",")), $this.raty(options)
            })
        }
    }
}), $.components.register("select2", {
    mode: "default",
    defaults: {width: "style"}
    ...
    mode: "init",
    defaults: {color: $.colors("primary", 600)},
    init: function (context) {
    ...
    mode: "init", init: function (context) {
        $.fn.matchHeight && $(".nav-tabs-vertical", context).each(function () {
    ...
    mode: "api", api: function () {
        $(document).on("change.site.task", '[data-role="task"]', function () {
            var $list = $(this), $checkbox = $list.find('[type="checkbox"]');
            $checkbox.is(":checked") ? $list.addClass("task-done") : $list.removeClass("task-done")
    ...
    mode: "init", init: function (context) {
        if ($.fn.toolbar) {
            ...
            $('[data-plugin="toolbar"]', context).each(function () {
                var $this = $(this), content = $this.data("toolbar"), options = $.extend(!0, {}, defaults);
                content && (options.content = content), $this.toolbar(options)
            })
        }
    }
